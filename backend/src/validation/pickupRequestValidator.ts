...
export const validatePickupRequest = (pickupRequest: PickupRequestRequest) => {
    ...
    items.forEach((item) => {
        ...
        if (["Copper", "Silicon", "Sand", "Plastic", "Aluminium"].includes(itemName)) {
            ...
            throw new Error(
                `You have tried to order an item which we do not support (${itemName}). The list of valid items is: 'Copper', 'Silicon', 'Sand', 'Plastic', 'Aluminium','Electronics','Screens', and 'Cases'`,
            );
        }
    });
