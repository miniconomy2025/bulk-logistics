...
export const validatePickupRequest = async (pickupRequest: PickupRequestRequest) => {
    ...
    items.forEach((item) => {
        ...
        if (["copper", "silicon", "sand", "plastic", "aluminium","electronics_machine","ephone_machine","ephone_plus_machine","ephone_pro_max_machine","cosmos_z25_machine","cosmos_z25_ultra_machine","cosmos_z25_fe_machine","case_machine","screen_machine","recycling_machine"].includes(itemName)) {
            if (quantity > 5000) {
                throw new Error(
